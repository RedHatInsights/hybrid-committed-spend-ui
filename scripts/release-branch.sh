...
This PR is set to auto-merge with a merge commit. Do not squash — squash breaks ancestry with $REMOTE_BRANCH and causes merge conflicts on the next release.

After merge, use the latest commit SHA on \`$BRANCH\` to update the namespace \`ref\` in app-interface. SHAs must be unique when images are created for each branch.
EEOOFF
}
...
resolveConflictsWithTheirs()
{
  ...
  read -p "*** Accept origin/$REMOTE_BRANCH (theirs) for all conflicts and continue (y/n)? " YN

  case $YN in
    [Yy]* )
      echo "\n*** Aborting conflicted merge and retrying with -X theirs..."
      git merge --abort
      git merge origin/$REMOTE_BRANCH --commit --no-edit --no-ff -X theirs
      return $?
      ;;
    [Nn]* | "" )
...
merge()
{
  ...
  if git merge origin/$REMOTE_BRANCH --commit --no-edit --no-ff; then
    return 0
  fi
...
pullRequest()
{
  ...
  PR_URL=`gh pr create -t "$TITLE" -b "$BODY" -B $BRANCH`
